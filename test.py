...
def get_ipa_file_path():
    #工作目录下面的 ipa 文件
    ipa_file_workspace_path = '/Users/allanwendy/data/jenkins/jobs/jenkins/workspace/app/build/outputs/apk/' + 'app-debug.apk'
...
def send_Email(json_result):
    ...
    mail_pwd = 'wenyu1984757'
    mail_to = ','.join(mail_receiver)
    
    msg = MIMEMultipart()
    
    environsString = '<h3>本次打包相关信息</h3><p>'
    # environsString += '<p>apk 包下载地址 : ' + 'wudizhi' + '<p>'
    ...
    msg['To'] = mail_to
    msg['from'] = mail_user
    msg['subject'] = '最新Android平台apk打包文件已完成，构建版本号:' + jenkins_build_number
