...
public class MergeAdapter extends BaseAdapter implements SectionIndexer {
    ...
    public int getItemRawType(int position) {
        int result = -1;
        ...
        for (PieceState piece : pieces.getRawPieces()) {
            if (piece.isActive) {
                ...
                if (position < size) {
                    result = piece.adapter.getItemViewType(position);
                    break;
                }
    ...
    public void pendingActive(ListAdapter adapter, boolean isActive) {
        pieces.setActive(adapter, isActive);
    }

    public void pendingActive(View v, boolean isActive) {
        pieces.setActive(v, isActive);
    }

    public boolean isActive(View view) {
        return pieces.isActive(view);
    }

    public boolean isActive(ListAdapter adapter) {
        return pieces.isActive(adapter);
    }
    ...
    private static class PieceStateRoster {
        ...
        boolean isActive(ListAdapter adapter) {
            for (PieceState state : pieces) {
                if (state.adapter == adapter) {
        ...
        boolean isActive(View view) {
            for (PieceState state : pieces) {
                if (state.adapter instanceof SackOfViewsAdapter
                        && ((SackOfViewsAdapter) state.adapter).hasView(view)) {
                    return state.isActive;
                }
            }
            return false;
